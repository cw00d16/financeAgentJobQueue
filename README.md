...
- **Web search usage is capped separately from the token budget** (`RESEARCH_MAX_SEARCH_USES`, default 5) — added after deployment, not part of the original design. Web search carries its own flat fee ($10 per 1,000 searches) that never shows up in token usage at all, so the token budget above doesn't actually bound it. Found this out directly: an early uncapped test job did ~10 searches (close to Anthropic's own per-call limit) and cost ~$0.15, while token-based cost tracking alone reported only ~$0.05 for that same job — a ~3x undercount. Capping `web_search`'s `max_uses` directly also closes a compounding path the iteration cap alone didn't: without it, a job that wants more than ~10 searches gets `pause_turn` and the loop resumes it for another ~10, up to `RESEARCH_MAX_ITERATIONS` times — a real route to 30–50 searches on a stubborn job. Below the cap, Claude just writes with what it has instead of asking for more, so the research phase reliably finishes in one call.
...
- Measured cost per CI trigger (each push to `main`, and each push to an open PR against it): ~$0.006 for the extraction eval (3 fixtures, no search involved), ~$0.06–$0.10 for the research eval (3 fixtures, judge-graded) — roughly $0.07–$0.11 total, real Claude spend every time the pipeline runs.
